...
def train(cfg, gen_net: nn.Module, dis_net: nn.Module, gen_optimizer, dis_optimizer, gen_avg_param, train_loader,
    ...
    for iter_idx, imgs in enumerate(tqdm(train_loader)):
        ...
        z = torch.cuda.FloatTensor(np.random.normal(0, 1, (imgs.shape[0], cfg['latent_dim']))).cuda()
        ...
        if cfg['loss'] == 'hinge':
            ...
            if isinstance(fake_validity, list):
                ...
                d_loss = d_real_loss + d_fake_loss
        elif cfg['loss'] == 'wgangp':
            gradient_penalty = compute_gradient_penalty(dis_net, real_imgs, fake_imgs.detach(), cfg['phi'])
            d_loss = -torch.mean(real_validity) + torch.mean(fake_validity) + gradient_penalty * 10 / (
                    cfg['phi'] ** 2)
        elif cfg['loss'] == 'wgangp-mode':
            gradient_penalty = compute_gradient_penalty(dis_net, real_imgs, fake_imgs.detach(), cfg['phi'])
            d_loss = -torch.mean(real_validity) + torch.mean(fake_validity) + gradient_penalty * 10 / (
                    cfg['phi'] ** 2)
        elif cfg['loss'] == 'wgangp-eps':
            gradient_penalty = compute_gradient_penalty(dis_net, real_imgs, fake_imgs.detach(), cfg['phi'])
            d_loss = -torch.mean(real_validity) + torch.mean(fake_validity) + gradient_penalty * 10 / (
                    cfg['phi'] ** 2)
        ...
        d_loss = d_loss/float(cfg['accumulated_times'])
        d_loss.backward()
        
        if (iter_idx + 1) % cfg['accumulated_times'] == 0:
        ...
        #  Train Generator
        # -----------------
        if global_steps % (cfg['n_critic'] * cfg['accumulated_times']) == 0:
            
            for accumulated_idx in range(cfg['g_accumulated_times']):
                gen_z = torch.cuda.FloatTensor(np.random.normal(0, 1, (cfg['gen_batch_size'], cfg['latent_dim'])))
                gen_imgs = gen_net(gen_z)
                fake_validity = dis_net(gen_imgs)

                # cal loss
                loss_lz = torch.tensor(0)
                if cfg['loss'] == "standard":
                    real_label = torch.full((cfg['gen_batch_size'],), 1., dtype=torch.float, device=real_imgs.get_device())
                    fake_validity = nn.Sigmoid()(fake_validity.view(-1))
                ...
                if cfg['loss'] == "lsgan":
                    ...
                    fake_image1, fake_image2 = gen_imgs[:cfg['gen_batch_size']//2], gen_imgs[cfg['gen_batch_size']//2:]
                    z_random1, z_random2 = gen_z[:cfg['gen_batch_size']//2], gen_z[cfg['gen_batch_size']//2:]
                ...
                g_loss = g_loss/float(cfg['g_accumulated_times'])
                g_loss.backward()
            ...
            ema_nimg = cfg['ema_kimg'] * 1000
            cur_nimg = cfg['dis_batch_size'] * cfg['world_size'] * global_steps
            if cfg['ema_warmup'] != 0:
                ema_nimg = min(ema_nimg, cur_nimg * cfg['ema_warmup'])
                ema_beta = 0.5 ** (float(cfg['dis_batch_size'] * cfg['world_size']) / max(ema_nimg, 1e-8))
            else:
                ema_beta = cfg['ema']
        ...
        # verbose
        if gen_step and iter_idx % cfg['print_freq'] == 0:
...
#             save_image(sample_imgs, f'sampled_images_{args.exp_name}.jpg', nrow=4, normalize=True, scale_each=True)
            ...
            tqdm.write(
                ...
                (epoch, cfg['max_epoch'], iter_idx % len(train_loader), len(train_loader), d_loss.item(), g_loss.item(), ema_beta))
            del gen_imgs
            del real_imgs
            del fake_validity
            del real_validity
...
def save_samples(cfg, fixed_z, epoch, gen_net: nn.Module):
    ...
    with torch.no_grad():
        ...
        os.makedirs(f"./samples/{cfg['exp_name']}", exist_ok=True)
        save_image(sample_imgs, f"./samples/{cfg['exp_name']}/sampled_images_{epoch}.png", nrow=10, normalize=True, scale_each=True)
    return 0


def save_samples_plots(cfg, fixed_z, epoch, gen_net: nn.Module):

    # eval mode
    gen_net.eval()
    
    z = torch.cuda.FloatTensor(np.random.normal(0, 1, (64, cfg['latent_dim']))).cuda()
    fake_imgs = gen_net(z).cpu().detach().numpy()

    for i in range(10):
        
        # Plot all waves from the DataFrame
        t = range(0, len(fake_imgs[0][0][0]))
        plt.figure(figsize=(12, 6))
        
        plt.title(f'Epoch {epoch}|Plot {i}: Synthetic data validation')
        plt.xlabel('Time (s)')
        plt.ylabel('Amplitude')
        for j in range(0, len(fake_imgs[0])):
            plt.plot(t, fake_imgs[i][j][0], label=f'Wave {j}')
        
        # Configure the plot
        plt.grid(True)
        plt.legend()
        
        save_path = f"{cfg['path_helper']['sample_path']}/epoch{epoch}/"
        os.makedirs(save_path, exist_ok=True)
        plt.savefig(save_path + f"Fig{i}.png")
