...
class Generator(nn.Module):
    def __init__(self, seq_len=150, patch_size=15, channels=3,  latent_dim=100, embed_dim=10, depth=3,
                 num_heads=8, forward_expansion=4, forward_drop_rate=0.5, attn_drop_rate=0.5):
    ...
    def forward(self, z):
        ...
        H, W = 1, self.seq_len
        x = self.blocks(x)
        x = x.reshape(x.shape[0], 1, x.shape[1], x.shape[2]) #is this correct?
...
class Discriminator(nn.Sequential):
    def __init__(self, 
        ...
        super().__init__(
            ...
            Dis_TransformerEncoder(depth, emb_size=emb_size, num_heads=num_heads, drop_p=drop_p,
                                   forward_expansion=forward_expansion, forward_drop_p=forward_drop_p, **kwargs),
